...
from opfl import OPFLScorer
from opfl.excel_parser import find_position_rows, parse_player_name
...
def print_discrepancies(
    discrepancies: list[dict], total_checked: int, output: OutputWriter, verbose: bool = True
):
    ...
    if mismatches:
        ...
        output.write('  ' + '-' * 80)
        output.write(
            f'  {"Pos":<4} {"Player":<30} {"Matched":<20} {"Excel":>7} {"Calc":>7} {"Diff":>7}'
        )
        output.write('  ' + '-' * 80)
        ...
        for d in mismatches:
            ...
            matched_str = (
                d.get('matched_name', '')[:20] if d.get('matched_name') != d['player'] else ''
            )
            output.write(
                f'  {d["position"]:<4} {player_str:<30} {matched_str:<20} {d["excel_score"]:>7.1f} {d["calculated_score"]:>7.1f} {d["difference"]:>+7.1f}'
            )
            if verbose and d['breakdown']:
                breakdown_str = ', '.join(
                    f'{k}: {v}' for k, v in d['breakdown'].items() if k != 'floor_applied'
                )
    ...
    if not_found:
        ...
        for d in not_found:
            player_str = f'{d["player"]} ({d["nfl_team"]})'[:30]
            output.write(
                f'  {d["position"]:<4} {player_str:<30} {d["excel_score"]:>7.1f} (bye/injured/no stats)'
            )
        output.write('')
        ...
        if args.all:
            ...
            for sheet_name, week_num in weeks:
                ...
                if args.summary:
                    ...
                    not_found = sum(
                        1 for d in discrepancies if d['reason'] == 'Player not found in stats'
                    )
                    matched = checked - len(discrepancies)
                    pct = (matched / checked * 100) if checked > 0 else 0
                    output.write(
                        f'  {checked} players: {matched} matched ({pct:.1f}%), {mismatches} mismatches, {not_found} not found'
                    )
                else:
            ...
            total_not_found = sum(
                1 for d in all_discrepancies if d['reason'] == 'Player not found in stats'
            )
