...
def update_lineup_file(week: int, team: str, starters: dict, github_token: str, locked_players: list = None) -> tuple[bool, str]:
    ...
    if locked_set and current_team_lineup:
        final_starters = {}
        # OPFL positions: QB, RB, WR, TE, K, DF, HC (no OL)
        for pos in ["QB", "RB", "WR", "TE", "K", "DF", "HC"]:
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.send_header("Access-Control-Allow-Origin", "*")
        self.end_headers()
        self.wfile.write(json.dumps({"status": "OPFL API is running", "method": "GET"}).encode())
    ...
    def do_POST(self):
        ...
        try:
            ...
            # OPFL positions (no OL position)
            valid_positions = ["QB", "RB", "WR", "TE", "K", "DF", "HC"]
            max_starters = {"QB": 1, "RB": 2, "WR": 2, "TE": 1, "K": 1, "DF": 1, "HC": 1}
