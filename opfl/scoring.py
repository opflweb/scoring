...
ScoreBreakdown = dict[str, float | bool]


def score_qb(stats: dict, turnover_tds: dict | None = None) -> tuple[float, ScoreBreakdown]:
    """
...
def score_rb_wr(stats: dict, turnover_tds: dict | None = None) -> tuple[float, ScoreBreakdown]:
    """
...
def score_te(stats: dict, turnover_tds: dict | None = None) -> tuple[float, ScoreBreakdown]:
    """
...
def score_kicker(stats: dict) -> tuple[float, ScoreBreakdown]:
    """
...
def score_head_coach(
    game_info: dict, spread_info: dict | None = None
) -> tuple[float, ScoreBreakdown]:
    """
    ...
    Scoring (based on ESPN spread):
        ...
        - Home underdog win: 5 pts
        - Road favorite win: 6 pts
    ...
    if team_score > opponent_score:
        ...
        if is_home:
            if is_underdog:
                points = 5  # Home underdog win
                breakdown["home_underdog_win"] = 5
            else:
            ...
            if is_underdog:
                ...
                points = 6  # Road favorite win
                breakdown["road_favorite_win"] = 6
    else:
